...
BlockchainDouble.prototype.processNextBlock = function(timestamp, callback) {
  ...
  this.createBlock(function(err, block) {
    ...
    let refinedGasEstimate = {};
    self.vm.on("step", (stepInfo) => {
      const { fee, name } = stepInfo.opcode;
      const addr = stepInfo.address.toString("hex");
      const codeHash = stepInfo.account.codeHash.toString("hex");
      if (refinedGasEstimate[addr] === undefined) {
        refinedGasEstimate[addr] = fee;
        console.log(`\nNEW: ${codeHash}, ${addr}, ${name}, ${fee}, ${refinedGasEstimate[addr]}\n`);
      } else {
        refinedGasEstimate[addr] += fee;
        console.log(`${codeHash}, ${addr}, ${name}, ${fee}, ${refinedGasEstimate[addr]}`);
      }
    });
  });
};
