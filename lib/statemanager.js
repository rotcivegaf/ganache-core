...
function StateManager(options, provider) {
  ...
  if (options.estimateGasThreshold <= 0) {
    options.estimateGasThreshold = 1;
  }
...
StateManager.prototype.processGasEstimate = function(from, tx, blockNumber, callback) {
  ...
  self.createTransactionWithCorrectNonce(tx, from, function(err, tx) {
    ...
    async.series([setLo, binarySearch], function(err) {
      if (err) {
        if (err === "0x0") {
        ...
        if (took > 1000 && self.estimateGasThreshold < DEFAULT_ESTIMATE_GAS_THRESHOLD) {
          self.logger.log(`Calculating gas estimate took: ${took} ms and ${iterations} iterations. \
          You may want to increase the value of the estimateGasThreshold option to speed up the calculation.`);
        }
        hi = Math.floor(hi);
      ...
      self.blockchain.processCall(tx, blockNumber, function(err, results) {
        ...
        async.doWhilst(
          function(cb2) {
            ...
            self.blockchain.processCall(tx, blockNumber, function(err, results) {
              let gasUsed = err ? self.blockchain.blockGasLimit : to.number(results.gasUsed);
